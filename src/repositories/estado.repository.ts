...
const modelRepository = AppDataSource.getRepository(Estado)

async function index()  {
    const response = await modelRepository.find({ relations: ['pais','created_by', 'updated_by']});
    return response;
}

async function find(id: number){
    const response = await modelRepository.findOne({where: {id: id}, relations: ['pais','created_by', 'updated_by']});
    return response;
}
