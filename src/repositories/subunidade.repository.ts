...
const modelRepository = AppDataSource.getRepository(Subunidade)

async function index()  {
    const response = await modelRepository.find({ relations: ['unidade','created_by', 'updated_by']});
    return response;
}

async function find(id: number){
    const response = await modelRepository.findOne({where: {id: id}, relations: ['unidade','created_by', 'updated_by']});
    return response;
}
