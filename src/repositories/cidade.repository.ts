...
const modelRepository = AppDataSource.getRepository(Cidade)

async function index()  {
    const response = await modelRepository.find({ relations: ['estado','created_by', 'updated_by']});
    return response;
}

async function find(id: number){
    const response = await modelRepository.findOne({where: {id: id}, relations: ['estado','created_by', 'updated_by']});
    return response;
}
