...
const modelRepository = AppDataSource.getRepository(Setor)

async function index()  {
    const response = await modelRepository.find({ relations: ['subunidade','created_by', 'updated_by']});
    return response;
}

async function find(id: number){
    const response = await modelRepository.findOne({where: {id: id}, relations: ['subunidade','created_by', 'updated_by']});
    return response;
}
